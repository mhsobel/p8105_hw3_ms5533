...
instacart_data %>%
  ...
  summarise(number = n()) %>% 
  ggplot(aes(x = aisle_id, y = number, color = department)) +
...
instacart_data %>% 
  ...
  summarise(
    n = n()) %>% 
  spread(key = aisle_id, value = n) %>% 
  arrange(desc(n)) %>% 
  top_n(5)

instacart_data %>% 
  filter(aisle %in% c("baking ingrediants", "dog food care", "packeged vegetables fruits"))
```
