...
include the day of the week orders were made (`order_dow`), the hour of
the day the order was made (`order_hour_of_day`), product name
(`product_name`), aisle of the product (`aisle`), and department the
product is in (`department`). Histograms (shown above) show that: Sunday
is the most popular day, midday is the most time of day, most people
wait a full month before ordering, and perishables are the most popular
...
![](p8105_hw3_ms5533_files/figure-gfm/unnamed-chunk-7-1.png)<!-- -->
...
instacart_data %>% 
  filter(aisle == "baking ingrediants" | aisle == "dog food care" | 
           aisle == " packeged vegetables fruits") %>% 
  group_by(aisle, product_name) %>% 
  summarise(
    n = n()) %>% 
  spread(key = aisle, value = n) %>% 
  arrange(desc(n)) %>% 
  top_n(5)
```
